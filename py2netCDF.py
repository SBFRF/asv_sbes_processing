        ...
        if var in data_dict:
            try:
                if "fill_value" in template_vars[var] and "least_significant_digit" in template_vars[var]:
                    ...
                    new_var = ncfile.createVariable(
                        template_vars[var]["name"], template_vars[var]["data_type"], template_vars[var]["dim"]
                    )
                ...
                if var == "station_name":
                    ...
                    assert (
                        data_dict[var].shape == new_var.shape
                    ), "The data must have the Same Dimensions  (missing time?)"
                    x = np.empty(
                        [data_dict[var].shape[0], data_dict[var].shape[1], data_dict[var].shape[2]], np.float64
                    )
                    for i in range(data_dict[var].shape[0]):
                        x[i] = data_dict[var][i]
                    new_var[:, :, :] = x[:, :, :]
