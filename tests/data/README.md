...
├── mini_files/                  # Real 5-minute survey window used by CI tests (~27 MB)
│   └── 20231109/
│       ├── CORS/                # CORS base-station RINEX files (~4 MB)
│       ├── emlidRaw/            # Rover RINEX files from Emlid receiver (~15 MB)
│       ├── nmeadata/            # NMEA GPS sentence files (~3 MB)
│       ├── s500/                # Cerulean S500 sonar binary files (~4.5 MB)
│       └── figures/             # Output figures from a previous run (~3.5 MB)
...
**Contents:**
| Sub-directory | Contents | Size |
|---|---|---|
| `CORS/` | CORS base-station RINEX (*.23o, *.23n, *.23g, *.sp3) | ~4 MB |
| `emlidRaw/` | Emlid rover RINEX + PPK position (*.23O, *.pos) | ~15 MB |
| `nmeadata/` | NMEA GPS sentence text files (*.dat) | ~3 MB |
| `s500/` | Cerulean S500 sonar binary files (*.dat) | ~4.5 MB |

**CI / Large-data strategy:** The `mini_files/` tree is committed directly to git
because it represents a one-time reduced extract (≤30 MB) that keeps CI
self-contained. Full survey days (hundreds of MB per day) should be stored
externally (cloud bucket / artifact server) and fetched explicitly by jobs that
need them. Do not commit additional survey days to this directory.
