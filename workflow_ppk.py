...
def main(
    ...
    # Handle case-insensitive folder naming for sonar data (e.g. "s500" vs "S500", "ect-d032" vs "ECT-D032")
    if not os.path.isdir(fpathSonar):
        # Search all entries in datadir for a case-insensitive match on sonar_model
        try:
            entries = os.listdir(datadir)
        except OSError:
            entries = []
        for entry in entries:
            if entry.lower() == sonar_model.lower() and os.path.isdir(os.path.join(datadir, entry)):
                alt_path = os.path.join(datadir, entry)
                # Check for a numeric date subfolder inside the matched directory
                date_subfolders = [
